...
include('D:/xampp/htdocs/Uchhero/Project/content/connection.php'); // Adjust this path if necessary

// Get and sanitize the posted data
$email = mysqli_real_escape_string($conn, $_POST['email']);
$password = mysqli_real_escape_string($conn, $_POST['password']);
$role = mysqli_real_escape_string($conn, $_POST['role']); // Ensure to get the role from POST data

// Prepare and bind the SQL statement to check email, password, and role
$stmt = $conn->prepare("SELECT * FROM login WHERE email = '$email_name' AND role = 'a's"); // Use placeholders for the query
$stmt->bind_param("ss", $email, $role); // Bind email and role parameters
...
if ($result->num_rows > 0) {
  ...
  if (password_verify($password, $user['pass'])) {
    // Password is correct, store role in session and redirect
    $_SESSION['role'] = $user['role']; // Assuming there's a role field
    header("Location: http://localhost/Uchhero/Project/header/login/login_page.php"); // Redirect to the login page
    exit();
  } else {
    // Invalid password
    $_SESSION['error'] = "Invalid email or password.";
    header("Location: http://localhost/Uchhero/Project/header/login.php");
    exit();
  }
} else {
  // User not found
